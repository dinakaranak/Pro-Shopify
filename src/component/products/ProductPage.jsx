...
const ProductPage = () => {
  ...
  const handleAddToCart = async () => {
    ...
    if (product.sizeChart?.length > 0 && !selectedSize) {
    ...
    try {
      const response = await Api.post('/cart', {
        ...
        quantity: quantity,
        color: product.colors?.length > 0 ? selectedColor : undefined, 
        size: product.sizeChart?.length > 0 ? selectedSize : undefined 
      });
      ...
      // More user-friendly error message based on the error received from the API
    ...
    const fetchProduct = async () => {
      try {
        ...
        const enhancedProduct = {
          ...
          colors: response.data.colors || [], // Initialize as empty array if no colors from API
          sizeChart: response.data.sizeChart || [], // Initialize as empty array if no sizeChart from API
